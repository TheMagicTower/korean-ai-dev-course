...
for d in data:
 ...
 sections.append(f'<section id="{d["key"]}"><h2>{d["title"]}</h2>{shots}<p><a href="https://github.com/TheMagicTower/korean-ai-dev-course/tree/main/{d["path"]}">실행 가능한 소스와 README ↗</a></p><h3>직전 보고부터 다음 보고까지</h3><ul>{stages}</ul><details><summary>실제 작업 프롬프트 원문</summary><pre>{esc(prompt.read_text())}</pre></details>{records}{tests}<a href="{proof}events.jsonl">원본 이벤트 JSONL</a></section>')
...
병렬 실행의 시간 합은 사용자가 기다린 시간이 아닙니다. 공통 준비·검증 작업은 루트 기록에 분리했습니다. 정확한 런타임 모델 ID가 노출되지 않아 모델 간 속도 비교도 하지 않습니다. 과거 PoC에는 외부 자료 수집·API 시도가 포함됐고 이번 두 경로는 같은 기록 입력을 재사용했습니다. 따라서 순수 기능량만의 차이를 측정한 통제 실험도 아닙니다. 단일 사례·단일 실행이므로 ‘언제나 신규가 빠르다’거나 ‘프로덕션은 PoC의 몇 배다’라는 일반 결론을 낼 수 없습니다.
